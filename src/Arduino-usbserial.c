...
int main(void) {
    ...
    RingBuffer_InitBuffer(&USBtoUSART_Buffer, USBtoUSART_BufferData,
                          BUFFER_SIZE);
    RingBuffer_InitBuffer(&USARTtoUSB_Buffer, USBtoUSART_BufferData,
                          BUFFER_SIZE);
    ...
    for (;;) {
        ...
        if (!(RingBuffer_IsFull(&USBtoUSART_Buffer))) {
            ...
            /* Read bytes from the USB OUT endpoint into the USART transmit
             * buffer */
            if (!(ReceivedByte < 0))
